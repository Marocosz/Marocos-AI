...
Se a pergunta for "o que o Marcos sabe fazer", esta é a resposta curta. São quatro capacidades, na ordem em que o mercado as disputa. Cada uma começa pelo que muda para quem contrata, e só depois diz como é feita — e cada uma tem projeto no portfólio que a sustenta. Nenhuma delas é uma lista de tecnologia: tecnologia é ferramenta, capacidade é o que se contrata.

1. **Agentes de IA no formato que o problema pedir.** Agente não é um formato, é uma forma de organizar decisão, e o que eu entrego é conseguir escolher o formato certo — não um formato só. Já construí, e tenho projeto de cada um:

   - **conversacional** — chat com persona, memória e recuperação sobre dado real (o Marcos Virtual deste portfólio, e os agentes do motor de cobrança na Finza);
   - **algorítmico**, com o modelo entrando num passo só — o resto é código determinístico: text-to-SQL com roteador barato antes do modelo caro, guarda de SQL por árvore sintática, e um agente corretor que devolve o erro do banco ao modelo (o hub da Supporte);
   - **em lote sobre volume** — chamados viram embedding, UMAP reduz, HDBSCAN agrupa por densidade, e só então o LLM explica o que cada grupo tem em comum (o Scope Intelligence);
   - **com aprovação humana no meio** — cinco agentes sequenciais (planejador, redator, revisor, crítico, finalizador) com porta de validação entre eles, por WebSocket, até sair um DOCX (o de qualidade da Supporte);
   - **embutido no produto** — 18 agentes que não conversam: devolvem sugestão tipada com severidade e payload de ação, e o frontend desenha card e botão a partir disso (a Bússola V2);
   - **agendado** — relatório de segurança escrito pelo modelo sobre KPI calculado, gerado e arquivado sem ninguém pedir (o Portal de Controle de Acesso).

   O que não muda entre os formatos é a exigência: fluxo determinístico em código (grafo de estados, não swarm), saída **válida para uma máquina** e não só legível para uma pessoa — o que obriga contrato tipado, validação estrita e normalização da saída solta do modelo —, e uma guarda de responsabilidade que prefere dizer "não sei" a inventar. E onde há conversa, cada conversa é instrumentada, porque agente que ninguém mede é boato.

2. **Uma pessoa do primeiro requisito até a produção.** Você fala com uma pessoa da primeira conversa até o sistema no ar, e depois dele. Não existe a passagem de bastão onde o projeto cai: eu levanto o escopo, construo o backend e o frontend, publico em infraestrutura que eu mesmo administro, e continuo respondendo pelo que quebrar. É justamente essa última parte que a maioria dos freelances não faz. Por baixo: FastAPI com contrato tipado e migração de verdade, Postgres usado direito (tsvector e GIN antes de apelar para um segundo banco), React e TypeScript na frente, Docker e Coolify na minha VPS, sessão no servidor quando token não pode ser revogado, e auditoria escrita na mesma transação da mudança. Sustentam isso: o Portal Acadêmico, o Diário Oficial Eletrônico e o Portal do Cidadão.

3. **Automação onde o software não colabora.** O trabalho repetitivo quase nunca está num sistema que ajuda você a tirá-lo de lá. Eu automatizo também sobre software fechado de terceiro: mapeio o que ele guarda por fora e devolvo pelo canal que o próprio fornecedor oferece, nunca escrevendo nas tabelas dele. Você para de fazer à mão sem ter que trocar o sistema que já pagou. Por baixo: n8n e a API do WhatsApp para o canal, agendador para o que tem de acontecer sem requisição, e mapeamento de schema sem documentação por comparação de snapshots. Sustentam isso: o Portal de Controle de Acesso, e o trabalho diário na Finza.

4. **Velocidade com IA sem a dívida técnica que ela costuma trazer.** IA deixa qualquer um mais rápido; o que separa as pessoas é quanto ela sabe do seu código. Eu monto um harness por projeto — as convenções, os erros que já custaram depuração, e uma verificação que o agente não tem permissão de pular antes de afirmar que algo funciona. Rápido deixa de significar frágil, e isso vale para o repositório do cliente, não só para os meus. Por baixo: skills por repositório, subagentes com contexto isolado, hooks, servidores MCP ligados ao painel de deploy e ao banco de verdade, e trabalho por spec (especificação escrita, depois plano, depois execução). Sustentam isso: este portfólio (que tem uma skill escrita para ele) e o faculdAIde.
...
- Comida brasileira, arroz, feijao, macarrão alho e oleo, bife aceboladi, batata frita e banana
- Pizzas, em especial pizza "Crocante" com borda recheada de catupiry da pizzaria Tia Luzia perto da casa dos meus pais em Patrocínio
- Lasanha
- Churrasco

## Primeiros Contatos com Tecnologia
...
Meu primeiro celular, um Samsung Pocket, foi completamente explorado. Fiz root, testei ROMs, personalizei o sistema e aprendi, na prática, como o Android funcionava por dentro. Esse processo de experimentar, errar e aprender foi essencial para construir minha base técnica e minha forma de pensar.

## Início Profissional e Primeiros Projetos
...
Além da formação acadêmica, a faculdade me proporcionou experiências extremamente enriquecedoras fora da sala de aula. Participei, por quase um ano, de um projeto de extensão, no qual atuei como instrutor, ministrando aulas de informática para crianças no Centro Comunitário Sal da Terra. Essa experiência foi muito importante para o desenvolvimento da comunicação, da empatia e da responsabilidade social. Caso queira conhecer um pouco mais da Comunidade [Sal da Terra](https://www.missaosaldaterra.org.br/)
...
#### Lista dos meus Games Favoritos

1. Elden Ring
2. God Of War 2
3. Naruto Shippuden Ninja Ultimate 5
4. The Witcher 3
...
- **Interestelar:** Reflete meu interesse antigo pelo universo e pela astronomia. Quando criança, sempre me destaquei nas provas da Olimpíada Brasileira de Astronomia, chegando muito próximo das maiores premiações. Além do tema, sou grande admirador de Matthew McConaughey, especialmente por suas atuações em True Detective (1ª temporada) e Clube de Compras Dallas.
...
Caso queira ver um pouco mais da minha opiniao sobre filmes: [Meu Letterboxd](https://letterboxd.com/Marocosz/)
Caso queira ver um pouco mais sobre meus animes favoritos: [Meu anime planet](https://www.anime-planet.com/users/Marocos)
...
8. Hunter x Hunter
9. Pluto
10. Sword Art Online (Temporada 1 e 2 apenas)
...
Todos esses interesses funcionam como fontes constantes de inspiração e reflexão. Costumo pensar que cultura é a identidade que carregamos, a memória coletiva e individual que molda quem somos. Por isso, defendo e sempre defenderei o acesso à cultura em todas as suas formas.

#### Top bandas e Músicas
...
- **Python:** Nível Avançado (Minha linguagem principal). Experiência sólida em desenvolvimento backend, automação e ciência de dados.
- **JavaScript:** Forte. Utilizado tanto no desenvolvimento de interfaces reativas quanto em scripts de integração.
- **Java:** Iniciante. Conhecimentos fundamentais de orientação a objetos e backend.

### Backend

- **Frameworks:** FastAPI (Avançado/Expert), Flask. Foco em criação de APIs RESTful performáticas e escaláveis.
- **Ferramentas de IA/Orquestração:** LangChain (Avançado), LangGraph. Implementação de arquiteturas complexas baseadas em agentes.
...
- **Situação Atual:** Aberto a propostas de projetos Freelance (Fullstack/IA/Automação), estou empregado atualmente, porém se tiver algum proposta, entre em contato para conversarmos!!
- **Idiomas:**
  - Português: Nativo.
  - Inglês: [Intermediário/Avançado - Leitura técnica fluente, conversação em desenvolvimento].

## Meu Setup de Desenvolvimento

- **Sistema Operacional:** Windows 11
- **IDE:** VSCode + Antigravity.
- **Hardware:** Notebook Acer Nitro V15 com um Ryzen 7735Hs, RTX 4050 6 Gb VRAM, 32 Gb de RAM, um monitor extra.
- **Café:** Com açúcar
